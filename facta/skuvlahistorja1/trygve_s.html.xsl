...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:variable name="filename" select="'trygve_s.htm'"/>
  <xsl:variable name="title" select="'Norgga sámiskuvlla birra'"/>
  ...
  <xsl:variable name="translated_from" select="'nob_PLEASE_CORRECT_ME'"/>
  <xsl:variable name="publisher" select="'Davvi Girji'"/>
  <xsl:variable name="publChannel" select="''"/>
  <xsl:variable name="year" select="'2005'"/>
  <xsl:variable name="ISBN" select="''"/>
  <xsl:variable name="ISSN" select="''"/>
  <xsl:variable name="place" select="'http://girji.info/skolehist/trygve-s.htm'"/>
  ...
  <xsl:variable name="wordcount" select="'1715'"/>
...
<xsl:variable name="para_nob" select="'trygve_n.html'"/>
