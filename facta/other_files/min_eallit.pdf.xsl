...
<!-- Format query results for display --><xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:variable name="filename" select="'min_eallit.pdf'"/>
  <xsl:variable name="title" select="'Min eallig'"/>
  <xsl:variable name="author1_fn" select="'Eigil'"/>
  <xsl:variable name="author1_ln" select="'Eikvil'"/>
  ...
  <xsl:variable name="place" select="'http://www.skolenettet.no/nyUpload/Samisk læringsnett/Dokumenter/min eallit.pdf'"/>
  ...
  <xsl:variable name="wordcount" select="'976'"/>
