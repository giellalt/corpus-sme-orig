...
<xsl:stylesheet version="1.0" xmlns:i18n="http://apache.org/cocoon/i18n/2.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:variable name="filename" select="'elliid_orrunsadji.pdf'"/>
  <xsl:variable name="title" select="'Elliid orrunsadji'"/>
  <xsl:variable name="author1_fn" select="'Elisabeth'"/>
  <xsl:variable name="author1_ln" select="'Skjervum Hole'"/>
  ...
  <xsl:variable name="place" select="'http://www.skolenettet.no/nyUpload/Samisk læringsnett/Dokumenter/elliid orrunsadji.pdf'"/>
  ...
  <xsl:variable name="wordcount" select="'1030'"/>
  <xsl:variable name="metadata" select="'uncomplete'"/>
  <xsl:variable name="template_version" select="' 1.9 '"/>
  <xsl:variable name="current_version" select="'$Revision: 1.1 $'"/>
<!-- Free text field for notes -->
<xsl:variable name="note" select="'Excluded from conversion. The text cannot be extracted at all.'"/>
