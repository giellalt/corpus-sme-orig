...
<!-- Format query results for display --><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i18n="http://apache.org/cocoon/i18n/2.1" version="1.0">
    ...
    -->
    <xsl:variable name="filename" select="'http://samediggi.no/Artikkel.aspx?aid=3359&amp;sprak=Samisk&amp;Print=1'"/>
    <xsl:variable name="text_encoding" select="''"/>
    <xsl:variable name="title" select="'*Preassadieđahusat 2009-2013 - Sámediggi Sametinget'"/>
    <!--
    ...
    <xsl:variable name="wordcount" select="'2996'"/>
    ...
    <xsl:variable name="parallels">
        ...
        <parallel_text xml:lang="nob" location="samediggi-article-3359.html"/>
...
<xsl:element name="p">
            ...
            <xsl:call-template name="globalTextReplace">
               <xsl:with-param name="inputString" select="$text"/>
               <xsl:with-param name="target" select="'. Ja/EMRIP)d/S ámedigge/dii.13.00/1.b/1.b/'"/>
               <xsl:with-param name="replacement" select="'. ja/EMRIP) d/Sámedigge/dii. 13.00/1. b/'"/>
                <xsl:with-param name="continue" select="0"/>
            </xsl:call-template>
</xsl:element>
</xsl:template>
