...
<!-- Format query results for display --><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i18n="http://apache.org/cocoon/i18n/2.1" version="1.0">
    ...
    -->
    <xsl:variable name="filename" select="'http://www.manskligarattigheter.se/Media/Get/606/nordsamiska-pdf'"/>
    <xsl:variable name="text_encoding" select="''"/>
    <xsl:variable name="title" select="'Europaráđi dárkkisteapmi'"/>
    <!--
    ...
    <xsl:variable name="parallels">
        ...
        <parallel_text xml:lang="smj" location="europarade_guoradallam_delto.pdf"/>
    ...
    <xsl:variable name="bottom_margin" select="'1=17'"/>
...
<xsl:template match="p[parent::body][not(./em | ./span)][text()]">
        ...
        <xsl:element name="p">
            ...
            <xsl:call-template name="globalTextReplace">
                <xsl:with-param name="inputString" select="$text"/>
                <xsl:with-param name="target" select="'Ruo…a/Ruo……a/Ruo…as/nuortaruo…as/dea…álaš/Ruotta/'"/>
                <xsl:with-param name="replacement" select="'Ruoŧa/Ruoŧŧa/Ruoŧas/nuortaruoŧas/deaŧálaš/Ruoŧŧa/'"/>
                <xsl:with-param name="continue" select="0"/>
            </xsl:call-template>
        </xsl:element>
    </xsl:template>
