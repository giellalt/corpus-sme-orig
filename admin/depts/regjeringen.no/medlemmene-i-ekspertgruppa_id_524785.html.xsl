...
<!-- Format query results for display --><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i18n="http://apache.org/cocoon/i18n/2.1" version="1.0">
    ...
    -->
    <xsl:variable name="filename" select="'http://regjeringen.no/se/dep/hod/kampanjer/ovttasdoaibmanoastus-/medlemmene-i-ekspertgruppa.html?id=524785'"/>
...
<xsl:element name="p">
            ...
            <xsl:call-template name="globalTextReplace">
               <xsl:with-param name="inputString" select="$text"/>
               <xsl:with-param name="target" select="'mielke-ja/ovdala /ráđ lahttu/'"/>
               <xsl:with-param name="replacement" select="'mielke- ja/ovdal /ráđđelahttu'"/>
                <xsl:with-param name="continue" select="0"/>
            </xsl:call-template>
</xsl:element>
</xsl:template>
