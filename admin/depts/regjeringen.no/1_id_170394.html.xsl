...
<!-- Format query results for display --><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i18n="http://apache.org/cocoon/i18n/2.1" version="1.0">
    ...
    -->
    <xsl:variable name="filename" select="'http://regjeringen.no/se/dep/kkd/dokumeanttat/proposiuvnnat-ja-dieahusat/otprp/20012002/od-prp-nr-114_2001-2002/1/1.html?id=170394'"/>
    <xsl:variable name="text_encoding" select="''"/>
    <xsl:variable name="title" select="'1.1 Proposišuvnna váldosisdoallu - regjeringen.no'"/>
    <!--
...
<xsl:element name="p">
            ...
            <xsl:call-template name="globalTextReplace">
               <xsl:with-param name="inputString" select="$text"/>
               <xsl:with-param name="target" select="'má.gga/'"/>
               <xsl:with-param name="replacement" select="'máŋgga/'"/>
                <xsl:with-param name="continue" select="0"/>
            </xsl:call-template>
</xsl:element>
</xsl:template>
