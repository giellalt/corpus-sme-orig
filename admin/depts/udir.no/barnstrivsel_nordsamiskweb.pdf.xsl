...
<!-- Format query results for display --><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i18n="http://apache.org/cocoon/i18n/2.1" version="1.0">
    ...
    -->
    <xsl:variable name="filename" select="'http://www.udir.no/globalassets/upload/barnehage/pedagogikk/veiledere/barnstrivsel_nordsamiskweb.pdf'"/>
    ...
    <!--
        ...
        1, 6-10, 15, 20, 25-30
    -->
    <xsl:variable name="skip_pages" select="'1,3,6,10,14,16,19,20,22,26,31,32'"/>
    ...
    <xsl:variable name="top_margin" select="'all=5,12=18,24=48'"/>
    <xsl:variable name="bottom_margin" select="'2=16,8=15,17=57'"/>
    ...
    <xsl:variable name="inner_top_margin" select="'15=64'"/>
    <xsl:variable name="inner_bottom_margin" select="'15=24'"/>
...
<xsl:template match="p[parent::body][not(./em | ./span)][text()]">
        ...
        <xsl:element name="p">
            ...
            <xsl:call-template name="globalTextReplace">
                <xsl:with-param name="inputString" select="$text"/>
                <xsl:with-param name="target" select="'baga dusa/soapmá sat'"/>
                <xsl:with-param name="replacement" select="'bagadusa/soapmásat'"/>
                <xsl:with-param name="continue" select="0"/>
            </xsl:call-template>
        </xsl:element>
    </xsl:template>
